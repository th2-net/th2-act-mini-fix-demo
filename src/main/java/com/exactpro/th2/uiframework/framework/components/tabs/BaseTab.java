...
public abstract class BaseTab<T extends BaseTab<T>> extends WinUIElement {
	protected WinLocator tabLocator;

	public BaseTab(WinBuilderManager builders) {
		super(builders);
	}


	public T init(WinLocator tabLocator) {
		this.tabLocator = tabLocator;
		return getTab();
	}

	protected abstract T getTab();
}
